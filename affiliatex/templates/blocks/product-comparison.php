								...
								<div class="affx-versus-product">
									<?php if ( $pcImage ) : ?>
										<div class="affx-versus-product-img">
											<?php echo wp_kses_post( AffiliateX_Helpers::affiliatex_get_media_image_html( $item['imageId'] ?? 0, $item['imageUrl'] ?? '', $item['imageAlt'] ?? '' ) ); ?>
										</div>
									<?php endif; ?>
										...
										<div>
											...
											<?php if ( $pcRating ) : ?>
												<div class="affx-rating-wrap">
													<?php echo \AffiliateX\Helpers\AffiliateX_Helpers::kses( $this->render_pc_stars( $item['rating'], $starColor, $starInactiveColor ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
												</div>
											<?php endif; ?>
											<?php if ( $pcButton ) : ?>
